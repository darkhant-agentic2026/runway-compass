...
| Q4 | Should research reports accumulate per task (history) or should a re-run replace the previous report? | ~~M4~~ **settled** | Accumulate, newest shown by default, older ones collapsible. Cheap and non-destructive. A re-run *replaces* the task's item checklist, though — see [02-data-model.md](02-data-model.md#task-items) — because two reports' worth of items is a checklist nobody can finish. **M8 changes the *mechanism*, not the answer**: reports still accumulate (one document per run, never overwritten), but each now lives behind its own run's research view rather than an inline "N earlier runs" disclosure on the task workspace — the workspace shows only the latest as a card. Older runs are still in Firestore and still reachable by URL; there is no browsable list of them yet, which is a smaller UI than the disclosure gave and a deliberate scope cut for the M8 slice rather than a reversal of this answer. |
